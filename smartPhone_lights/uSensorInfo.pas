...
uses
  ...
  System.Sensors, FMX.StdCtrls, FMX.Controls.Presentation, sgcWebSocket_Classes,
  ...
  FMX.ScrollBox, FMX.Memo,sgcWebSocket_Protocol_MQTT_Message;
...
procedure TfrmAboutSensors.Button1Click(Sender: TObject);
begin
  ...
  sgcWSPClient_MQTT1.Subscribe('lighth');
end;

procedure TfrmAboutSensors.Button2Click(Sender: TObject);
begin
  sgcWebSocketClient1.Active := false;
end;

procedure TfrmAboutSensors.Button3Click(Sender: TObject);
begin
//  if pos(() > 0) then
...
function TfrmAboutSensors.GetFullInfo(ACategory, AType, AClass, AAvailibleProperties: string): string;
begin
  ...
  begin
    case LProp of
      ...
      begin
          ...
          sgcWSPClient_MQTT1.Publish('lighth3', FloatToStr(ls.Lux));
      end;
...
function TfrmAboutSensors.GetInfoAboutLocation(ASensor: TCustomSensor): string;
var
...
function TfrmAboutSensors.GetInfoAboutMechanical(ASensor: TCustomSensor): string;
var
...
function TfrmAboutSensors.GetInfoAboutOrientation(ASensor: TCustomSensor): string;
var
...
function TfrmAboutSensors.GetTypeNameElectro(AType: TElectricalSensorType): string;
begin
  Result := cND;
  case AType of
...
function TfrmAboutSensors.GetTypeNameEnv(AType: TEnvironmentalSensorType): string;
begin
  Result := cND;
  case AType of
...
function TfrmAboutSensors.GetTypeNameOrientation(AType: TOrientationSensorType): string;
begin
  case AType of
...
procedure TfrmAboutSensors.ReAlignComponents;
begin
...
procedure TfrmAboutSensors.sgcWSPClient_MQTT1MQTTConnect(
...
procedure TfrmAboutSensors.sgcWSPClient_MQTT1MQTTDisconnect(
...
procedure TfrmAboutSensors.Timer2Timer(Sender: TObject);
begin
  sgcWSPClient_MQTT1.Publish('abcde', LValues);
  Memo1.Lines.Add(LValues);
end;
